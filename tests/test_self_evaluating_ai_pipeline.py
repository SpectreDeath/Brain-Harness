"""Test suite for self-evaluating-ai-pipeline skill and Knowledge Vault item."""
...
@pytest.mark.unit
class TestSelfEvaluatingAIPipelineSkill:
    ...
    def test_card_ascii_formatting_and_pillars(self, skill_dir: Path) -> None:
        ...
        assert (
            "SKILL:       self-evaluating-ai-pipeline" in card_text
            or "SKILL: self-evaluating-ai-pipeline" in card_text
        )

        node = SkillCardParser.parse_directory(skill_dir)
        assert node is not None
        assert node.name == "self-evaluating-ai-pipeline"
    ...
    def test_zero_fork_config(self, skill_dir: Path) -> None:
        ...
        assert "evaluation_thresholds" in text
        assert "layer1_deterministic_pass_rate" in text
    ...
    def test_knowledge_vault_dual_file_format(self, ki_dir: Path) -> None:
        ...
        assert meta["id"] == "ki_self_20260917_02"
        ...
        assert "Three-Layer LLM Evaluation Architecture" in summary_text
        assert "Deterministic Sanity Guardrails" in summary_text
        assert "Anchored Rubric" in summary_text
        assert "Cohen's Kappa" in summary_text
        assert "Paired Student's t-Test" in summary_text
