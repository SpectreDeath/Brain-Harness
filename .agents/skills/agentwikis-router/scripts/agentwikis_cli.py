...
from agentwikis_engine import AgentWikisEngine, DocumentSlice, MatchResult, QualityScorecard
...
def get_engine(corpus_dir: str | None = None, base_url: str = "https://agentwikis.com") -> AgentWikisEngine:
...
def cmd_search(args: argparse.Namespace) -> int:
    ...
    print(f"Success! Search returned {len(selected)} results (Confident: {has_confident}). Written to: {out_path}")
    return 0
...
def cmd_context_pack(args: argparse.Namespace) -> int:
    ...
    print(f"Success! Prepared context pack ({len(pack_md)} chars). Written to: {out_path}")
    return 0
...
def cmd_read_doc(args: argparse.Namespace) -> int:
    ...
    print(f"Success! Read '{doc_slice.title}' ({len(doc_slice.content)} chars, Source: {doc_slice.source_isnad}). Written to: {out_path}")
    return 0
...
def cmd_validate_contract(args: argparse.Namespace) -> int:
    ...
    engine = get_engine(args.corpus_dir, args.base_url)
    contract_path = Path(args.contract) if args.contract else Path(__file__).resolve().parent.parent / "contracts" / "agentwikis_contract.yaml"
    ...
    print(f"Success! Contract status: [{status_str} | {len(report.violations)} violations]. Written to: {out_path}")
...
def cmd_quality_profile(args: argparse.Namespace) -> int:
    ...
    print(f"Success! Quality Scorecard: [{status_str} | Overall: {scorecard.overall_score:.1f}%]. Written to: {out_path}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--corpus-dir", help="Path to AgentWikis corpus directory", default=None)
    parser.add_argument("--base-url", help="Base URL for remote AgentWikis API", default="https://agentwikis.com")

    subparsers = parser.add_subparsers(dest="subcommand", required=True)

    # list-wikis
    p_list = subparsers.add_parser("list-wikis", help="List wikis filtered by category or tags")
    p_list.add_argument("--category", help="Filter by category")
    p_list.add_argument("--tag", help="Filter by tag")
    p_list.add_argument("--query", help="Filter by text query in slug, title, description")
    ...
    # match-skill
    p_match = subparsers.add_parser("match-skill", help="Match user task to in-scope wikis and skills")
    p_match.add_argument("query", help="Task description or prompt")
    p_match.add_argument("--limit", type=int, default=3, help="Max candidates to return")
    p_match.add_argument("--output", required=True, help="Output JSON filepath")
    p_match.set_defaults(func=cmd_match_skill)

    # search
    p_search = subparsers.add_parser("search", help="Search corpus with calibrated confidence")
    ...
    p_read.add_argument("doc_path", help="Relative document path (e.g. vllm/wiki/index.md)")
    ...
    # validate-contract
    p_contract = subparsers.add_parser("validate-contract", help="Validate corpus against Open Data Contract")
    p_contract.add_argument("--contract", help="Optional custom contract YAML path", default=None)
    p_contract.add_argument("--output", required=True, help="Output JSON filepath")
    p_contract.set_defaults(func=cmd_validate_contract)

    # quality-profile
    p_quality = subparsers.add_parser("quality-profile", help="Run DAMA 6-dimension data quality profile")
    p_quality.add_argument("--min-score", type=float, default=85.0, help="Minimum passing score")
    p_quality.add_argument("--output", required=True, help="Output filepath (.json or .md)")
    p_quality.set_defaults(func=cmd_quality_profile)

    # mcp-config
    p_mcp = subparsers.add_parser("mcp-config", help="Generate MCP client setup configuration")
    p_mcp.add_argument("--client", default="antigravity", choices=["antigravity", "claude", "claude-code", "json"])
    ...
    # context-pack
    p_pack = subparsers.add_parser("context-pack", help="Compile one-shot token-bounded context pack")
    p_pack.add_argument("query", help="Task description or prompt")
    p_pack.add_argument("--max-tokens", type=int, default=4000, help="Token budget bound")
